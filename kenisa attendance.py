...
# ===================== الإعدادات العامة =====================
# قراءة بيانات الاعتماد من ملف secrets.toml (يجب وضعه في .streamlit/)
def get_credentials():
    try:
...
class Database:
    ...
    def _df_to_sheet(self, sheet_name, df, columns):
        """كتابة DataFrame إلى ورقة العمل مع الحفاظ على الأعمدة."""
        ...
        if not df.empty:
            ...
            ws.update([columns])  # فقط الأعمدة بدون بيانات
...
def logout():
    ...
    if st.session_state.user:
        db = Database(get_credentials(), SPREADSHEET_ID)
...
def show_login_page(db: Database, salt: str, jwt_secret: str):
        ...
        with st.form("login_form"):
            ...
            if submitted:
                if not username or not password:
                    ...
                    users = db.get_users()
                    if users.empty:
                        st.error("لا يوجد مستخدمين في النظام. الرجاء إضافة مسؤول أول.")
                    else:
    ...
    with tab2:
        ...
        passwd = st.text_input("كلمة مرور الاختبار", type="password", placeholder="أدخل كلمة المرور الخاصة بالاختبار")
                ...
                if quizzes.empty:
                    ...
                    if quiz.empty:
                        ...
                        if pd.to_datetime(quiz["expiry_date"]) < datetime.now():
                            ...
                            # إخفاء الاختبار تلقائياً بتحديث حالته
...
def show_dashboard(db: Database):
    ...
    st.markdown("#### 📈 تحليل الحضور (آخر 7 أيام)")
    if not attendance.empty:
        ...
        if not recent.empty:
            fig = px.histogram(recent, x="date", color="status", barmode="group",
                               title="الحضور اليومي", labels={"date": "التاريخ", "count": "العدد"})
    ...
    if absent_today > 5:
        st.warning(f"عدد الغائبات اليوم {absent_today} طالبة، يرجى المتابعة.")
    if not followup.empty:
        ...
        if not urgent.empty:
            st.error(f"هناك {len(urgent)} طالبة منقطعة عن المتابعة!")
...
def show_admin_users(db: Database, salt: str):
    ...
    with st.expander("✏️ تعديل / حذف مستخدم"):
        if not users.empty:
            ...
            selected_user_id = st.selectbox("اختر المستخدم", user_ids,
                                             format_func=lambda x: users[users.user_id==x]["full_name"].values[0])
            user_row = users[users.user_id == selected_user_id].iloc[0]
            new_role = st.selectbox("الصلاحية", ["System Admin", "Father Account", "Service Manager", "Teacher"],
                                    index=["System Admin", "Father Account", "Service Manager", "Teacher"].index(user_row["role"]))
            if st.button("تحديث الصلاحية"):
                db.update_user(selected_user_id, {"role": new_role})
                db.add_log(st.session_state.user["user_id"], f"تعديل صلاحية المستخدم {user_row['username']}")
                st.success("تم التحديث")
                st.rerun()
            if st.button("حذف المستخدم", type="primary"):
                db.delete_user(selected_user_id)
                db.add_log(st.session_state.user["user_id"], f"حذف المستخدم {user_row['username']}")
                st.success("تم الحذف")
                st.rerun()
...
def show_students_management(db: Database):
    ...
    with st.expander("➕ إضافة طالبة جديدة"):
        with st.form("add_student_form"):
            col1, col2 = st.columns(2)
            full_name = col1.text_input("الاسم الكامل")
            section = col2.selectbox("القسم", sections["section_id"] if not sections.empty else [])
            teacher = st.selectbox("المدرسة المسؤولة عن الافتقاد", teachers["user_id"] if not teachers.empty else [],
    ...
    # حذف أو تعديل
    with st.expander("✏️ تعديل أو حذف طالبة"):
        if not students.empty:
            selected = st.selectbox("اختر طالبة", students["student_id"],
                                    format_func=lambda x: students[students.student_id==x]["full_name"].values[0])
            st.info("يمكنك تعديل البيانات يدوياً في Google Sheet أو استخدام واجهة التعديل (قريباً).")
            if st.button("حذف الطالبة"):
                db.delete_student(selected)
                st.success("تم الحذف")
                st.rerun()
...
def show_teachers_sections(db: Database):
    ...
    with st.expander("➕ إضافة قسم"):
        with st.form("add_section"):
            ...
            manager = st.selectbox("مدير القسم", db.get_users()[db.get_users().role=="Service Manager"]["user_id"].tolist(),
                                   format_func=lambda x: db.get_users()[db.get_users().user_id==x]["full_name"].values[0])
            if st.form_submit_button("إضافة"):
                db.add_section({"section_id": str(uuid.uuid4()), "section_name": name, "manager_user_id": manager})
                st.success("تمت الإضافة")
                st.rerun()

    st.markdown("---")
    st.subheader("توزيع المدرسات على الطالبات")
    students = db.get_students()
    teachers = db.get_users()[db.get_users().role.isin(["Teacher"])]
    if not teachers.empty:
        for _, teacher in teachers.iterrows():
            t = teacher.to_dict()
            assigned = students[students.teacher_id == t["user_id"]]
            st.write(f"**{t['full_name']}**: {len(assigned)} طالبة")
...
def show_attendance(db: Database):
    ...
    if sections.empty:
        ...
        return
    section = st.selectbox("اختر القسم", sections["section_id"], format_func=lambda x: sections[sections.section_id==x]["section_name"].values[0])
...
def show_followup(db: Database):
    ...
    # فلترة الطالبات المسؤول عنهن المستخدم
    if user_role == "Teacher":
        ...
        responsible = students  # Admin/Father يرى الكل
    ...
    if not followups.empty:
        ...
        if not student_fups.empty:
            st.dataframe(student_fups[["followup_date", "followup_type", "notes", "regularity_status"]], use_container_width=True)
        else:
...
def show_quizzes(db: Database):
    ...
    if user_role in ["System Admin", "Service Manager"]:
        ...
        with st.form("quiz_form"):
            ...
            if st.form_submit_button("إنشاء الاختبار"):
                if not title:
                    ...
                    st.info("احفظ الكود وكلمة المرور لتوزيعها على الطالبات.")
                    st.rerun()
        ...
        if not quizzes.empty:
            ...
            quiz_choice = st.selectbox("اختر اختباراً لإضافة/عرض الأسئلة", active_quizzes["quiz_id"],
    ...
    if not results.empty:
        ...
        # ترتيب
        if st.button("عرض ترتيب الطالبات"):
            top = merged.groupby("student_name")["score"].sum().reset_index().sort_values("score", ascending=False)
            st.dataframe(top, use_container_width=True)
    else:
        st.info("لا توجد نتائج اختبارات بعد.")
...
def main():
    ...
    try:
        ...
        st.error("لم يتم العثور على بيانات اعتماد Google. تأكد من ملف .streamlit/secrets.toml")
        return
