...
def show_attendance_dashboard(db: Database):
    """Display attendance statistics and dashboard."""
    ...
    if "status" in today_att.columns:
        present_count = len(today_att[today_att.status == "حاضر"])
        absent_count = len(today_att[today_att.status == "غائب"])
        late_count = len(today_att[today_att.status == "متأخر"])
    else:
        present_count = absent_count = late_count = 0
    
    # KPI cards
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("✅ حاضرون اليوم", present_count)
    col2.metric("❌ غائبون اليوم", absent_count)
    col3.metric("🕐 متأخرون اليوم", late_count)
    col4.metric("👥 إجمالي الطالبات", len(students))
    ...
    if not attendance.empty and "date" in attendance.columns:
        ...
        week_att = attendance[attendance.date >= last_week.strftime("%Y-%m-%d")]
        
        if not week_att.empty and "status" in week_att.columns:
            st.subheader("📊 الحضور الأسبوعي")
            daily = week_att.groupby(["date", "status"]).size().reset_index(name="count")
            fig = px.line(daily, x="date", y="count", color="status", 
                          labels={"date": "التاريخ", "count": "عدد الطالبات", "status": "الحالة"},
                          color_discrete_map={"حاضر": "#28a745", "غائب": "#dc3545", "متأخر": "#ffc107"})
            fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig, use_container_width=True)
    ...
    if not attendance.empty and "section_id" in attendance.columns:
        st.subheader("📊 الحضور حسب الفصول")
        if not sections.empty and "section_id" in sections.columns:
            section_att = attendance.merge(sections[["section_id", "section_name"]], on="section_id", how="left")
            section_att = section_att.rename(columns={"section_name": "الفصل"})
        else:
            section_att = attendance.copy()
            section_att["الفصل"] = section_att["section_id"]
        
        if "status" in section_att.columns:
            section_summary = section_att.groupby(["الفصل", "status"]).size().reset_index(name="count")
            section_pivot = section_summary.pivot(index="الفصل", columns="status", values="count").fillna(0)
            if "حاضر" in section_pivot.columns and "غائب" in section_pivot.columns:
                section_pivot["معدل_الحضور"] = (section_pivot["حاضر"] / (section_pivot["حاضر"] + section_pivot["غائب"])) * 100
