...
class Database:
    ...
    def add_section(self, sec_data: dict):
        ...
        # نضمن وجود عمود manager_user_id فارغ للحفاظ على الهيكل
        if "manager_user_id" not in df.columns:
            df["manager_user_id"] = ""
    ...
    with st.expander("➕ إضافة مستخدم جديد"):
        with st.form("add_user_form"):
            ...
            if role in ["Service Manager", "Teacher"]:
                ...
                section = st.selectbox("الفصل", section_options)
...
def show_teachers_management(db: Database):
    ...
    with st.expander("➕ إضافة مدرسة جديدة"):
        with st.form("add_teacher_form"):
            ...
            sections = db.get_sections()
            section_options = ["None"] + (sections["section_id"].tolist() if not sections.empty else [])
            section = st.selectbox("الفصل المسؤولة عنه", section_options,
    ...
    with st.expander("✏️ تعديل / حذف مدرسة"):
        if not teachers.empty:
            ...
            sections = db.get_sections()
            section_options = ["None"] + (sections["section_id"].tolist() if not sections.empty else [])
            current_section = teacher_data.get("section_id", "")
            current_section_index = section_options.index(current_section) if current_section in section_options else 0
            new_section = st.selectbox("الفصل المسؤولة عنه", section_options,
                                       format_func=lambda x: sections[sections.section_id==x]["section_name"].values[0] if x != "None" and not sections.empty else "لا يوجد",
            ...
            if col1.button("تحديث البيانات", use_container_width=True):
                update_data = {
                    ...
                    "phone": new_phone,
                    "email": new_email,
                    "section_id": new_section if new_section != "None" else ""
                }
                db.update_user(selected_teacher, update_data)
                st.success("تم التحديث")
                time.sleep(1)
                st.rerun()
...
def show_students_management(db: Database):
    ...
    with st.expander("✏️ تعديل بيانات طالبة"):
        if not students.empty:
            ...
            current_section = student_row.get("section_id", "")
            section_index = section_options.index(current_section) if current_section in section_options else 0
            new_section = st.selectbox("الفصل", section_options,
                                       format_func=lambda x: sections[sections.section_id==x]["section_name"].values[0] if not sections.empty else x,
                                       index=section_index)
            ...
            if st.button("حفظ التعديلات", use_container_width=True):
                db.update_student(selected_student, {
                    "full_name": new_full_name, "section_id": new_section,
